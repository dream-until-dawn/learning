...
class DQNLearning:
    def __init__(
        ...
        self.optimizer_value = torch.optim.Adam(self.model_value.parameters(), lr=1e-2)
    ...
    def train_value(self, state, reward, next_state, over):
        ...
        for _ in range(10):
            # 计算value
            value: torch.Tensor = self.model_value(state)
    ...
    def train_action(self, state, action, value):
        ...
        # 更新前的动作概率
        with torch.no_grad():
            prob_old = self.model_action(state).gather(dim=1, index=action)

        # 每批数据反复训练10次
        for _ in range(10):
            # 更新后的动作概率
            prob_new = self.model_action(state).gather(dim=1, index=action)

            # 求出概率的变化
            ratio: torch.Tensor = prob_new / prob_old

            # 计算截断的和不截断的两份loss,取其中小的
            surr1: torch.Tensor = ratio * delta
            surr2: torch.Tensor = ratio.clamp(0.8, 1.2) * delta

            loss: torch.Tensor = -torch.min(surr1, surr2).mean()
