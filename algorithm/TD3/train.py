...
class DQNLearning:
    def __init__(
        ...
        model_value1_delay: nn.Sequential,
        model_value2: nn.Sequential,
        model_value2_delay: nn.Sequential,
        player: Player,
        pool: Pool,
    ):
        self.player = player
        ...
        self.model_value1_delay = model_value1_delay
        self.model_value2 = model_value2
        self.model_value2_delay = model_value2_delay
        ...
        self.requires_grad(self.model_value1_delay, False)
        self.requires_grad(self.model_value2_delay, False)
    ...
    def requires_grad(self, model: nn.Sequential, value: bool):
    ...
    def train_action(self, state):
        ...
        value1: torch.Tensor = self.model_value1(input)
        value2: torch.Tensor = self.model_value2(input)
    ...
    def train_value(self, state, action, reward, next_state, over):
        ...
        # 计算value
        input = torch.cat([state, action], dim=1)
        value1 = self.model_value1(input)
        value2 = self.model_value2(input)

        # 计算target
        next_action = self.model_action_delay(next_state)
        input = torch.cat([next_state, next_action], dim=1)
        with torch.no_grad():
            target1 = self.model_value1_delay(input)
            target2 = self.model_value2_delay(input)
    ...
    def train(
        ...
        for epoch in range(200):
            ...
            self.soft_update(self.model_value1, self.model_value1_delay)
            self.soft_update(self.model_value2, self.model_value2_delay)
...
if __name__ == "__main__":
    if False:
        ...
        dqnLearning = DQNLearning(
            ...
            dqnModel.model_value1_delay,
            dqnModel.model_value2,
            dqnModel.model_value2_delay,
            player,
            pool,
        )
        dqnLearning.train()
